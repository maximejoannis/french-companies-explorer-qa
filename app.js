...
const S={page:1,perPage:20,total:0,results:[],favorites:load("fce_favorites",[]),history:load("fce_history",[]),compare:load("fce_compare",[]),favoriteMeta:load("fce_favorite_meta",{}),saved:load("fce_saved",[]),sort:"relevance",theme:localStorage.getItem("fce_theme")||"light"};
...
$("#searchForm").onsubmit=e=>{e.preventDefault();search(1)};$("#queryInput").addEventListener("input",renderQueryHint);renderQueryHint();
...
$("#clearHistoryBtn").onclick=()=>{S.history=[];save("fce_history",S.history);renderHistory()};$("#sortSelect").onchange=()=>{sortResults();renderResults();renderStats();renderActiveFilters();syncUrl()};$("#saveSearchBtn").onclick=saveCurrentSearch;$("#exportJsonBtn").onclick=exportJson;$("#exportCsvBtn").onclick=exportCsv;
...
function renderStats(){const root=$("#statsPanel"),c=getVisibleCompanies();if(!c.length){root.classList.add("hidden");root.innerHTML="";return}const active=c.filter(x=>x.status==="A").length,closed=c.length-active,cities=new Set(c.map(x=>x.city).filter(Boolean)).size,work=c.filter(x=>x.workforce&&x.workforce!=="Non renseigné").length,postal=c.filter(x=>x.postalCode).length,oldest=c.map(x=>x.creation).filter(x=>/^\d{4}-\d{2}-\d{2}/.test(x)).sort()[0]||"—";root.innerHTML=`<div class="stats-extended"><article><span class="muted">Affichées</span><strong>${c.length}</strong></article><article><span class="muted">En activité</span><strong>${active}</strong><small class="muted">${closed} cessée(s)</small></article><article><span class="muted">Communes distinctes</span><strong>${cities}</strong><small class="muted">${postal} avec code postal</small></article><article><span class="muted">Effectif renseigné</span><strong>${work}</strong><small class="muted">Création la plus ancienne : ${esc(oldest)}</small></article></div>`;root.classList.remove("hidden")}
...
function detectQueryType(v){v=String(v||"").trim();if(/^\d{9}$/.test(v))return{type:"siren",valid:true,label:"SIREN détecté"};if(/^\d{14}$/.test(v))return{type:"siret",valid:true,label:"SIRET détecté"};if(/^\d+$/.test(v))return{type:"numeric-invalid",valid:false,label:"Identifiant invalide : un SIREN contient 9 chiffres et un SIRET 14 chiffres"};return{type:"text",valid:!!v,label:v?"Recherche textuelle":"Saisis un nom, un mot-clé, un SIREN ou un SIRET"}}
function renderQueryHint(){const h=$("#queryHint");if(!h)return;const d=detectQueryType($("#queryInput").value);h.textContent=d.label;h.className=`input-hint ${d.valid?"valid":d.type==="numeric-invalid"?"invalid":""}`}
function activeFilterData(){return[["cp","Code postal",$("#postalCodeFilter").value.trim()],["city","Commune",$("#cityFilter").value.trim()],["status","État",$("#statusFilter").value],["size","Taille",$("#pageSizeFilter").value!=="20"?$("#pageSizeFilter").value:""],["sort","Tri",$("#sortSelect")?.value&&$("#sortSelect").value!=="relevance"?$("#sortSelect").value:""]].filter(([, ,v])=>v)}
function renderActiveFilters(){const r=$("#activeFilters");if(!r)return;const items=activeFilterData();if(!items.length){r.classList.add("hidden");r.innerHTML="";return}r.classList.remove("hidden");r.innerHTML=items.map(([k,l,v])=>`<span class="filter-chip">${l}: ${esc(v)} <button data-remove-filter="${k}" aria-label="Supprimer ${esc(l)}">×</button></span>`).join("")+`<button id="resetActiveFilters" class="button secondary">Réinitialiser les filtres</button>`;r.querySelectorAll("[data-remove-filter]").forEach(b=>b.onclick=()=>removeFilter(b.dataset.removeFilter));$("#resetActiveFilters").onclick=resetFiltersOnly}
function removeFilter(k){if(k==="cp")$("#postalCodeFilter").value="";if(k==="city")$("#cityFilter").value="";if(k==="status")$("#statusFilter").value="";if(k==="size")$("#pageSizeFilter").value="20";if(k==="sort"&&$("#sortSelect"))$("#sortSelect").value="relevance";renderActiveFilters();if($("#queryInput").value.trim())search(1);else syncUrl()}
function resetFiltersOnly(){$("#postalCodeFilter").value="";$("#cityFilter").value="";$("#statusFilter").value="";$("#pageSizeFilter").value="20";if($("#sortSelect"))$("#sortSelect").value="relevance";renderActiveFilters();if($("#queryInput").value.trim())search(1);else syncUrl()}
function valueOrMissing(v,l="Non renseigné"){return v&&String(v).trim()&&!["Non renseignée","Non renseigné"].includes(v)?esc(v):`<span class="missing-data">— ${esc(l)}</span>`}
function favoriteMetaFor(s){return S.favoriteMeta[s]||{note:"",tags:[]}}
function saveFavoriteMeta(s,n,t){S.favoriteMeta[s]={note:n||"",tags:t||[]};save("fce_favorite_meta",S.favoriteMeta)}
function normalizeTags(v){return[...new Set(String(v||"").split(",").map(x=>x.trim()).filter(Boolean))].slice(0,8)}
function officialUrl(s){return`https://annuaire-entreprises.data.gouv.fr/entreprise/${encodeURIComponent(s)}`}
function establishmentStatus(e){return(e.etat_administratif||e.etat_administratif_etablissement||"").toUpperCase()}
function filteredEstablishments(c,m){const a=c.matchingEstablishments||[];if(m==="active")return a.filter(e=>establishmentStatus(e)==="A");if(m==="closed")return a.filter(e=>establishmentStatus(e)!=="A");return a}

async function search(page){
 const q=$("#queryInput").value.trim();const detected=detectQueryType(q);renderQueryHint();if(!q)return showState("Entre un terme de recherche.");if(!detected.valid)return showState(detected.label);
 ...
 try{
  ...
  $("#resultsTitle").textContent=`Résultats pour “${q}”`;$("#resultCount").textContent=`${S.total} résultat(s)`;$("#searchState").classList.add("hidden");renderResults();renderPagination();renderStats();renderActiveFilters();
...
window.addEventListener("popstate",()=>{const r=restoreFromUrl();if(r.hasQuery){route("search");search(r.page||1)}else route("home")});
